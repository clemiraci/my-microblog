...
@login_required
def user(username):
    user = User.query.filter_by(username=username).first_or_404() # returns eror if user was not found
    posts = [
...
@app.before_request # makes function executed before every view function
def before_request():
    if current_user.is_authenticated:
